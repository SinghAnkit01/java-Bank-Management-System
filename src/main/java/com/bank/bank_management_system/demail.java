...
public class demail extends JFrame {
    String pin,card,email,number,bank,branch,name2;
    static int balance;
    demail(String card,String pin,String email,String number,String bank,String branch, String name2,int balance){
        this.pin= pin;
        this.card=card;
        this.email=email;
        this.number=number;
    ...
    Date date = new Date();
    conn conn = new conn();
    String query = "Insert into emailstat(pin,date,type,amount,status)values('"+pin+"','"+date+"','Deposit','"+number+"','success')";
    conn.s.executeUpdate(query);

    ResultSet rs3 =  conn.s.executeQuery("select accountno from passbook where cardnumber ='" +card+"' and pin = '"+pin+"' ");
    if(rs3.next()){
        String account = rs3.getString("accountno");
        System.out.println(account);

    String query1 = "Insert into deposit(atmid,accountno,cardnumber,pin,Deposit_amount,balance_amount,status)values('ATM41410','"+ account +"','"+ card + "','" + pin +"','" + number +"','" + balance+"','SUCCESS')";
